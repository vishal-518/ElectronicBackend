...
app.get("/product/:productId", verifyToken, async (req, res) => {
  ...
  if (!user || !user.location)
    return res.json({ status: false, message: "User location not found" });

  const storeLocation = {
    latitude: Number(product.location.lat),
    longitude: Number(product.location.log),
  };
  const userLocation = {
...
app.post("/login", async (req, res) => {
  ...
  if (!exitsuser.isVerified) {
    return res.json({
      status: 400,
      msg: "Email not verified. Please verify OTP first.",
    });
  }
  ...
  let token = jwt.sign(
    { id: exitsuser.id, email: exitsuser.email, role: exitsuser.role },
...
app.get("/profile", verifyToken, async (req, res) => {
  try {
    ...
    if (userObj.profilePic && !userObj.profilePic.startsWith("http")) {
      userObj.profilePic = `${req.protocol}://${req.get("host")}/uploads/${
        userObj.profilePic
      }`;
    }
...
app.post("/verify-payment", async (req, res) => {
  ...
  if (!razorpay_order_id || !razorpay_payment_id || !razorpay_signature) {
    return res
      .status(400)
      .json({ success: false, msg: "Missing payment details" });
  }
...
app.post("/order", verifyToken, async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ status: 500, msg: "Something went wrong", error: err.message });
  }
});
...
app.post("/check-pincode", async (req, res) => {
  ...
  if (!pincode || pincode.length !== 6) {
    return res
      .status(400)
      .json({ status: 400, success: false, msg: "Invalid Pincode" });
  }
  ...
  res.json({
    status: 200,
    success: true,
    msg: `Delivery available in ${location.districtName || "Unknown"}, ${
      location.stateName || "Unknown"
    }`,
    estimatedDays: 5,
    location: {
  ...
  try {
    ...
    if (status && ["approved", "rejected"].includes(status)) {
      if (!order.return || order.return.status !== "requested") {
        return res
          .status(400)
          .json({ msg: "No return request found to update status" });
      }
    ...
    if (reason === "other") {
      if (!extraReason || extraReason.toString().trim().length < 3) {
        return res
          .status(400)
          .json({ msg: "Please specify a valid reason for 'Other'" });
      }
